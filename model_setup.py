...
def build_and_compile(input_shape=(28, 28, 1), lr=0.001):
    """
    Function that builds the model instance and compiles it.
    In future work we will implement further customisation for the model architecture and compiling parameters.
...
def training_fit_loop(train_data_name: str, data_step: int, n: int, N=1, start_data=500, plot=True):
    """
    ...
    for j in range(N):
        ...
        df.loc[f"Experiment {j}:"] = [r_sq_exp, (m_exp, t_exp, b_exp),
        ...
        if N > 1 and plot:
            ...
            if r_sq_exp < r_sq_power:
                plot_fits(data_index_array, loss_array, [m_exp, t_exp, b_exp], "exp")
                glob_params.append([m_exp, t_exp, b_exp])

            else:
                plot_fits(data_index_array, loss_array, [m_power, t_power, b_power], "power")
